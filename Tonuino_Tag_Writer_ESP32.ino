...
// 0.1.4 Time display corrected, daylight saving time changeover implemented
//
...
const int timeZone = 1;                              // Winterzeit: CET = UTC+1
...
bool loadConfig() {
  if (SPIFFS.exists("/config.json")) {
      ...
      if (configFile) {
        DEBUG_PRINTLN("[0162] opened config file");
        size_t size = configFile.size();
        if (size > 1024) {
          DEBUG_PRINTLN("[0165] Config file size is too large");
          return false;
        }
        ...
        if (json.success()) {
          ...
          DEBUG_PRINTLN("[0183] Failed to parse config file");
          return false;
        }
      }
      else {
        DEBUG_PRINTLN("[0188] Failed to open config file");
        return false;
      }
  }
  return true;
...
{
  DEBUG_PRINTLN("[0201] Should save config");
...
{
  DEBUG_PRINTLN("[0211] Entered config mode");
...
time_t getNtpTime()
{
  ...
  DEBUG_PRINTLN("[0237] NTP: Transmit NTP Request");
  WiFi.hostByName(ntpserver, ntpServerIP);          // Lookup IP from Hostname
  DEBUG_PRINT("[0239] NTP: ");
  ...
  while (millis() - beginWait < 1500) {
    ...
    if (size >= NTP_PACKET_SIZE) {
      DEBUG_PRINTLN("[0248] NTP: Receive NTP Response");
...
//      return secsSince1900 - 2208988800UL + (timeZone * 60 * 60); // NTP Time - 70 Years + TimeZone Hours
      ...
      return getLocalTimeEurope(utc);
    }
  }
  DEBUG_PRINTLN("[0261] NTP: No NTP Response :-(");
  return 0;                                          // return 0 if unable to get the time
}

bool isSummerTimeEurope(time_t utc)
{
  tmElements_t tm;
  breakTime(utc, tm);
  int y = tm.Year + 1970;
  ...
  t.Year = CalendarYrToTm(y);
  t.Month = 3;
  t.Day = 31;
  t.Hour = 2;
  t.Minute = 0;
  t.Second = 0;
  time_t march31 = makeTime(t);
  while (weekday(march31) != 1) march31 -= SECS_PER_DAY;
  ...
  t.Minute = 0;
  t.Second = 0;
  time_t oct31 = makeTime(t);
  while (weekday(oct31) != 1) oct31 -= SECS_PER_DAY;

  return (utc >= march31 && utc < oct31);
}

time_t getLocalTimeEurope(time_t utc)
{
  if (isSummerTimeEurope(utc))
    return utc + 2 * SECS_PER_HOUR;
  return utc + 1 * SECS_PER_HOUR;
}
...
void buildFooter()
{
  String timeString = printDigits2(hour()) + ":" + printDigits2(minute()) + ":" + printDigits2(second());

  if (de_enabled) {
    htmlFooter = "      <div class='row'><div class='col-md-12'><em>Tonuino Tag Writer (TTW), Aktuelle Zeit: " + timeString + "</em></div></div>\n";
  }
  else {
    htmlFooter = "      <div class='row'><div class='col-md-12'><em>Tonuino Tag Writer (TTW), Current time: " + timeString + "</em></div></div>\n";
  }
...
{ 
  if (httpServer.method() == HTTP_GET) {
    DEBUG_PRINTLN("[0532] WEB: Connection established - /config");
    sendConfigPage("", "", 0, 200);
  } 
  else {
    DEBUG_PRINTLN("[0536] WEB: Connection established - /config (save)");
    }
  if (de_enabled) {
...
void sendConfigPage(String message, String header, int type, int httpcode)
{
  ...
  if (type == 1){                                              // Type 1 -> save data
    String message = "[0554] WEB: Number of args received: ";
    message += String(httpServer.args()) + "\n";
    for (int i = 0; i < httpServer.args(); i++) {
      message += "[0557] Arg " + (String)i + " â€“> ";
    ...
    if (validconf)
    {
      DEBUG_PRINTLN("[0580] SPI: save config.json...");
      DynamicJsonBuffer jsonBuffer;
      ...
      if (!configFile) {
        DEBUG_PRINTLN("[0590] SPI: failed to open config file for writing");
      }
      #ifdef DEBUG
        json.printTo(Serial);
      #endif
    ...
    if (SPIFFS.begin())
    {
      DEBUG_PRINTLN("[0603] SPI: mounted file system");
      if (SPIFFS.exists("/config.json"))
      {
        // file exists, reading and loading
        DEBUG_PRINTLN("[0607] SPI: reading config file");
        File configFile = SPIFFS.open("/config.json", "r");
        if (configFile) {
          DEBUG_PRINTLN("[0610] SPI: opened config file");
          size_t size = configFile.size();
          ...
          DynamicJsonBuffer jsonBuffer;
          JsonObject& json = jsonBuffer.parseObject(buf.get());
          DEBUG_PRINT("[0618] JSO: ");
          #ifdef DEBUG
            json.printTo(Serial);
          #endif
          if (json.success()) {
            DEBUG_PRINTLN("\n[0623] JSO: parsed json");
            ...
            DEBUG_PRINTLN("[0629] JSO: failed to load json config");
          }
        }
      }
    } else {
      DEBUG_PRINTLN("[0634] SPI: failed to mount FS");
    }
  }
...
void Handle_tonuino(){
  if (httpServer.method() == HTTP_GET) {
    DEBUG_PRINTLN("[0714] WEB: Connection established - /tonuino");
    ...
    if (card == 0) {
      DEBUG_PRINTLN("[0730] RFID: No card presented!");
      if (de_enabled) {
    ...
    DEBUG_PRINTLN("[0753] WEB: Connection established - /tonuino (write card)");
    ...
    if (card == 0) {
      DEBUG_PRINTLN("[0864] RFID: No card presented!");
      if (de_enabled) {
...
//    String message = "WEB: Neue Werte gesetzt über die control page: ";
    String message = "[0894] WEB: Set new values using control page: ";
    message += String(httpServer.args()) + "\n";
    for (int i = 0; i < httpServer.args(); i++) {
...
void writeblock(int sector, int block) {
  ...
  for (byte k = 0; k < NR_KNOWN_KEYS; k++) {
    ...
    status = (MFRC522::StatusCode) mfrc522.PCD_Authenticate(MFRC522::PICC_CMD_MF_AUTH_KEY_A, sector*4+3, &key, &(mfrc522.uid));
    if (status != MFRC522::STATUS_OK) {
      DEBUG_PRINTLN("[1325] RFID: wrong write key");
      DEBUG_PRINTLN(mfrc522.GetStatusCodeName(status));
    }
    else {
      DEBUG_PRINTLN("[1329] RFID: key accepted");
      // Write data to the block
      status = (MFRC522::StatusCode) mfrc522.MIFARE_Write(sector*4+block, dataBlock, 16);
      if (status != MFRC522::STATUS_OK) {
        DEBUG_PRINTLN("[1333] RFID: data can not be written");
      }
      else {
        DEBUG_PRINTLN("[1336] RFID: data written");
        break;
      }
    }
  }
...
String byteToHexStringRange(byte* data, byte firstbyte, byte lastbyte)
{
  ...
  DEBUG_PRINT("[1359] byteToHexStringRange: ");
  DEBUG_PRINTLN(str);
  return String(str);
}
...
//  pinMode(IRQ_PIN, INPUT_PULLUP);
  ...
  DEBUG_PRINTLN("[1478] configured pinModes");
  ...
  if (SPIFFS.begin(true)) {                                         // Mounting File System, true because of formatOnFail
    DEBUG_PRINTLN("[1487] mounted file system");
    if (!loadConfig()) {
      DEBUG_PRINTLN("[1489] Failed to load config");
    } else {
      DEBUG_PRINTLN("[1491] Config loaded");
    }
  }
  else {
    DEBUG_PRINTLN("[1495] failed to mount FS");
  }
  ...
  DEBUG_PRINTLN("[1517] WiFi connected! IP: " + ipToString(WiFi.localIP()) + " Hostname: " + String(host_name) + " NTP-Server: " + String(ntpserver));

  // save the custom parameters to FS
  if (shouldSaveConfig) {
    DEBUG_PRINTLN(" [1521] config...");
    DynamicJsonBuffer jsonBuffer;
    JsonObject& json = jsonBuffer.createObject();
    ...
    File configFile = SPIFFS.open("/config.json", "w");
    if (!configFile) {
      DEBUG_PRINTLN("[1529] SPI: failed to open config file for writing config");
    }
    #ifdef DEBUG
      json.printTo(Serial);
    #endif
  ...
  httpServer.on("/freemem", []() {
    DEBUG_PRINTLN("[1545] WEB: Connection established: /freemem : ");
  ...
  httpServer.on("/update", HTTP_POST, []() {            // OTA Update
    ...
    if (upload.status == UPLOAD_FILE_START) {
      ...
      DEBUG_PRINTLN("[1589] Update Failed Unexpectedly (likely broken connection)");
    }
  });
  ...
  if (ntp_enabled) {
      DEBUG_PRINTLN("[1597] NTP: Starting UDP");
      Udp.begin(NtpLocalPort);
      DEBUG_PRINT("[1599] NTP: Local port: ");
      // DEBUG_PRINTLN(Udp.localPort());
      DEBUG_PRINTLN("[1601] NTP: waiting for sync");
  ...
  DEBUG_PRINTLN("[1611] TTW V0.1.3 / ESP32 / (c) HaSch 2023");
  DEBUG_PRINTLN("[1612] Startup completed!");
}
